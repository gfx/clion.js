...
I.prototype.load_tables = function() { // from the "#~" stream
    ...
    //XXX([ valid_mask_hi.toString(2), valid_mask_lo.toString(2) ]);
...
I.prototype.load_names = function() {
    ...
    if(this.tables[C.TABLE_ASSEMBLY].rows) {
        this.assembly_name = this.load_cstring_from_string_heap(
            C.TABLE_ASSEMBLY, 0, C.ASSEMBLY_NAME
        );
    }
    this.module_name = this.load_cstring_from_string_heap(
        C.TABLE_MODULE, 0, C.MODULE_NAME
    );
};
...
I.prototype.metadata_string_heap = function(idx) {
    ...
    return this.read_cstring( this.heap_strings.data + idx );
};
I.prototype.metadata_decode_row = function(table_idx, idx) {
    var bin = this.raw_data;
    var t   = this.tables[table_idx];
    ...
    for(i = 0; i < t.fields.length; i++) {
        size = t.field_size[i];
        switch(size) {
        case 1: col[i] = bin.readUInt8(offset);
        case 2: col[i] = bin.readUInt16(offset);
        case 4: col[i] = bin.readUInt32(offset);
...
I.prototype.metadata_decode_row_col = function(table_idx, idx, col) {
    var bin = this.raw_data;
    var t   = this.tables[table_idx];
    var i, offset, size;
