...
I.prototype.seek_set = function(offset) {
    this._offset = offset;
};
I.prototype.seek_cur = function(offset) {
    this._offset += offset;
};
I.prototype.peek_u8 = function(offset) {
    return this.raw_data.getUint8(
        offset === undefined ? this._offset : offset);
};
I.prototype.peek_u16 = function(offset) {
    return this.raw_data.getUint16(
        offset === undefined ? this._offset : offset);
};
I.prototype.peek_u32 = function(offset) {
    return this.raw_data.getUint32(
        offset === undefined? this._offset : offset);
};
I.prototype.read_u8 = function() {
    return this.raw_data.getUint8(this._offset++);
};
I.prototype.read_u16 = function() {
    var v = this.raw_data.getUint16(this._offset, true);
    this._offset += 2;
    return v;
};
I.prototype.read_u32 = function() {
    var v = this.raw_data.getUint32(this._offset, true);
    this._offset += 4;
    return v;
};
I.prototype.read_u64 = function() {
    var quad = { };
...
// byte array
I.prototype.read_bytes = function(len, klass)  {
...
// NUL-ended string
I.prototype.read_str = function(max_len /* optional */) {
...
I.prototype.metadata_decode_blob = function(idx) {
    if( idx >= this.heap_blob.size ) return { };
    return this.metadata_decode_value( this.heap_blob.data + idx);
};
I.prototype.metadata_decode_value = function(offset) {
    var size;
    this.seek_set(offset);
...
I.prototype.metadata_typedef_from_method = function(idx) {
    var tdef = this.tables[C.TABLE_TYPEDEF],
        token_idx, col_idx, i, col;
    console.log(tdef);
    token_idx = this.metadata_token_index(idx);
    col_idx   = C.TYPEDEF_METHOD_LIST;

    if(this.uncompressed_metadata) {
        token_idx = this.search_ptr_table(C.TABLE_FIELD_POINTER, token_idx);
    }

    // XXX: performs linear search, but binary search can be better
    for(i = 0; i < tdef.rows; i++) {
        col = this.metadata_decode_row_col(tdef, i, col_idx);
        console.log([i, col, token_idx, this.load_cstring_from_string_heap(tdef, i, col_idx)]);
        if(col === token_idx) {
            return i;
        }
    }
...
I.prototype.guid = function() {
    ...
    for(i = 0; i < this.heap_guid.size; i++) {
        x = this.read_u8().toString(16).toUpperCase();
...
I.prototype.class_get_full = function(type_token, context) {
    var klass;
    if(this.dynamic) {
        TODO('dynamic');
    }
    ...
    case C.TOKEN_TYPE_SPEC:
        ...
        break;
    default: throw new ImvalidImage();
    }
    return klass;
};

I.prototype.get_table_entries = function(t) { // for debugging
    var i, j, a = new Array(t.rows),
        row, v, save;
    ...
    for(i = 0; i < t.rows; i++) {
        ...
        for(j = 0; j < t.fields.length; j++) {
            ...
            case MetaType.UINT32:
                ...
                break;
            case MetaType.STRING_IDX:
                t.field_size[j] == 2 ? this.read_u16() : this.read_u32();
                save = this.tell();
...
I.prototype.get_table = function(name) { // for debugging
...
I.prototype.method_signature = function(m) {
    var idx, sig, container,
        signature,
        can_cache_signature;
    if(m.signature) {
        return m.signature;
    ...
    idx        = this.metadata_token_index(m.token);
    sig_offset = this.metadata_decode_row_col(this.tables[C.TABLE_METHOD],
                                              idx - 1, C.METHOD_SIGNATURE);
    sig        = this.metadata_decode_blob(sig_offset);
    ...
    */
    ...
    if(m.iflags & C.METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) {
        can_cache_signature = false;
    }
    else if(m.flags & C.METHOD_ATTRIBUTE_PINVOKE_IMPL) {
        can_cache_signature = false;
    }
    else if(container) {
        can_cache_signature = false;
    }
    else if(this.metadata_method_has_param_attrs(idx)) {
        can_cache_signature = false;
    }


    if(!signature) {
        signature = this.metadata_parse_method_signature_full(
            container, idx, sig);
        if(!signature) {
            throw new Error("Failed to load method signature");
        }
        // TODO: cache method signature
    }
...
I.prototype.parse_method_signature_full = function(container, def, sig) {
...
I.prototype.run = function(args) {
    var method = this.get_method( this.get_entry_point() );
