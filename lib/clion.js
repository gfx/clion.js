...
Image.prototype.metadata_parse_mh_full = function(container, ptr) {
    ...
    if(local_var_sig_tok) {
        ...
        locals = this.metadata_blob_heap(cols[C.STANDALONE_SIGNATURE]);
        bsize  = locals.size;

        this.seek_set(locals.data);
        if( this.read_u8() !== 0x07 ) {
            W("wrong signature for locals blob");
        }
...
Method.prototype.get_header = function() {
    ...
    XXX(this);
    if(this.wrapper_type !== C.WRAPPER_NONE || this.sre_this) {
       assert(this.header);
       return this.header;
    }

    assert( image.metadata_token_table(this.token) === C.TABLE_this );

    idx = image.metadata_token_index(this.token);
    rva = image.metadata_decode_row_col(image.tables[C.TABLE_this],
                                        idx - 1, C.this_RVA);

    // TODO: verify this header
