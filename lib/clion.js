...
Invocation.init = function(parent, obj, args, retval, method) {
    this.parent         = parent;
    this.obj            = invocant;
...
Image.prototype.do_parse_type = function(t, container, temporary) {
    var ok = true, token, klass, etype;
    t.type = this.metadata_decode_value().size;
    switch(t.type) {
    case C.TYPE_VOID:
    case C.TYPE_BOOLEAN:
    case C.TYPE_CHAR:
...
Image.prototype.metadata_parse_type_internal = function(container, mode, opt_attrs, temporary, ptr)
{
    var type, cached,
        byref = false,
        pinned = false,
    ...
    this.do_parse_type(type, container, temporary);
    return type;
};
...
Image.prototype.metadata_parse_method_signature_full = function(container, def, sig)
{
    ...
    if(gen_param_count) {
        gen_param_count = this.metadata_decode_value().size;
    }
    param_count = this.metadata_decode_value().size;
...
Method.prototype.get_header = function() {
    ...
    if( (method.flags & (C.METHOD_ATTRIBUTE_ABSTRACT | C.METHOD_ATTRIBUTE_PINVOKE_IMPL) )
       || (method.iflags & (C.METHOD_IMPL_ATTRIBUTE_RUNTIME | C.METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL)) ) {
       return null;
    }
    ...
    assert( image.metadata_token_table(method.token) === C.TABLE_METHOD );

    idx = image.metadata_token_index(method.token);
    ...
    return this.metadata_parse_mh_full( method.get_generic_container(), loc );
};
...
Image.prototype.invoke = function(method, object, params) {
    ...
    result = new Object();
    frame.init(context.current_frame, obj, result, method);

    frame.exec_method_with_context(context);
};
// execute a method in VES (virtual execution system)
Invocation.prototype.exec_method_with_context = function(context) {
    var child_frame;
    var ip;
    var sp;
    var rtm;
    ...
    if(!frame.runtime_method.transformed) {
        frame.runtime_method.transform_method(context);
    }

    while(true) main_loop: switch(ip) {
    case I_NOP:
        break;
    }
};
RuntimeMethod.prototype.transform_method = function(context) {
    ...
    */
    ...
    assert( signature.param_count + signature.hasthis < 1000 );
