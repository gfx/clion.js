...
I.prototype.metadata_blob_heap = function(idx) {
    ...
    else if( (bin[ptr] & 0x40) == 0 ) {
        size = ((bin[ptr] & 0x3f) << 8) + bin[ptr+1];
        ptr += 2;
    }
    else {
        size = ((bin[ptr] & 0x1f) << 24) +
              (bin[ptr+1] << 16) +
            (bin[ptr+2] << 8) +
             bin[ptr+3];
        ptr += 4;
...
I.prototype.metadata_decode_row = function(t, idx) {
    ...
    for(i = 0; i < t.fields.length; i++) {
        size = t.field_size[i];
        switch(size) {
        case 1: col[i] = bin.readUInt8(offset);  break;
        case 2: col[i] = bin.readUInt16(offset); break;
        case 4: col[i] = bin.readUInt32(offset); break;
